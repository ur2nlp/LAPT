"""Experiment registry: extract, annotate, compare and audit run records.

Shared by LAPT and its sibling projects. Everything that differs between them is
a `RegistrySchema`: the environment-variable prefix and the three tables that say
which config sections to flatten, which keys to drop, and how to order the ones
that are left. Usage examples for `--help` come from the calling script's own
module docstring, passed to `main()` as `epilog`.
"""
...
class RegistrySchema:
    """Everything about a registry that is project-specific.

    Attributes:
        env_prefix: Prefix for the environment variables this CLI honours, so
            `LAPT` gives `$LAPT_OUTPUTS_DIR`. Keeping projects on separate
            variables is what lets two checkouts share one shell.
        extract_sections: (config section, prefix) pairs to flatten into a run's
            params. The prefix is stripped from the recorded key.
        skip_keys: Keys dropped during extraction -- non-scalar, path-like, or
            redundant with something already recorded.
        preferred_order: Display order for well-known params. Anything not
            listed sorts alphabetically after these.
        hidden_params: Params hidden from display unless asked for, usually
            because they are implied by another one.
        outputs_dir: Where runs live, when the environment does not say.
    """

    env_prefix: str
    extract_sections: tuple[tuple[str, str], ...]
    skip_keys: frozenset[str]
    preferred_order: tuple[str, ...]
    hidden_params: frozenset[str] = frozenset({"eval_steps", "logging_steps", "save_steps"})
    outputs_dir: str = "outputs"

    @property
    def outputs_dir_env_var(self) -> str:
        """Name of the environment variable that overrides `outputs_dir`."""
        return f"{self.env_prefix}_OUTPUTS_DIR"

    def default_outputs_dir(self) -> Path:
        """Resolve the outputs directory, letting the environment win."""
        return Path(os.environ.get(self.outputs_dir_env_var, self.outputs_dir))
...
def extract_params(config: dict, schema: RegistrySchema) -> tuple[str, dict]:
    ...
    for section_name, _ in schema.extract_sections:
        section = config.get(section_name, {})
        for key, value in section.items():
            if key in schema.skip_keys:
                continue
...
def diff_runs(
    ...
    preferred_set = set(schema.preferred_order)
    all_keys = [k for k in schema.preferred_order if k in seen_keys]
...
def cmd_extract(args: argparse.Namespace, schema: RegistrySchema) -> None:
    ...
    for config in configs_to_process:
        try:
            experiment_id, params = extract_params(config, schema)
...
def cmd_show(args: argparse.Namespace, schema: RegistrySchema) -> None:
    ...
    for rid in run_ids:
        ...
        hidden = schema.hidden_params | set(args.hide)
        preferred_set = set(schema.preferred_order)
        ordered_keys = [k for k in schema.preferred_order if k in params and k not in hidden]
...
def cmd_diff(args: argparse.Namespace, schema: RegistrySchema) -> None:
    ...
    varying, constant = diff_runs(registry, run_ids, schema)
...
def cmd_annotate(args: argparse.Namespace, schema: RegistrySchema) -> None:
...
def cmd_verify(args: argparse.Namespace, schema: RegistrySchema) -> None:
    ...
    for run_id in run_ids:
        ...
        try:
            ...
            _, extracted_params = extract_params(config, schema)
...
def cmd_debt(args: argparse.Namespace, schema: RegistrySchema) -> None:
...
def cmd_sort(args: argparse.Namespace, schema: RegistrySchema) -> None:
...
def main(schema: RegistrySchema, epilog: str | None = None) -> None:
    """Run the registry CLI against a project's schema.

    Args:
        schema: The calling project's `RegistrySchema`.
    ...
    parser.add_argument(
        "--outputs-dir",
        type=Path,
        default=schema.default_outputs_dir(),
        help=(
            "Directory holding registry.yaml, configs/ and trainer_states/ "
            f"(default: {schema.default_outputs_dir()}, "
            f"or ${schema.outputs_dir_env_var})"
        ),
    )
    ...
    show_parser.add_argument(
        ...
        help=(
            ...
            f"(always hides: {sorted(schema.hidden_params)})"
        ),
    )
    show_parser.add_argument(
        "--sort",
    ...
    commands[args.command](args, schema)
